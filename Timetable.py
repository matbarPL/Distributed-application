...
class Timetable():
    ...
    def setConflicts(self):
        ...
        greaterSub = [(x[0], x[1]) for x in list(
            dicSub.items()) if x[1] != x[0].timesPerWeek]
        greater = [(x[0], x[1])
                   for x in list(dic.items()) if x[1] > x[0].limit]

        self.conflicts += 3*sum([abs(x[1] - x[0].timesPerWeek)
                                 for x in greaterSub])
    ...
    def generate(self, path, openfile=True):
        ...
        cell_format1 = workbook.add_format(
            {'text_wrap': True, 'valign': 'vcenter', 'align': 'center', 'bold': True})
